...
export default function PostRow({ post }) {
  return (
    <Link
      to={`/blog/${post.slug}`}
      className="grid gap-2 border-b border-line py-5 transition hover:bg-card/80 md:grid-cols-[180px_1fr_auto] md:items-center md:gap-8"
    >
      <time dateTime={post.publishedDate} className="font-mono text-xs text-muted">
